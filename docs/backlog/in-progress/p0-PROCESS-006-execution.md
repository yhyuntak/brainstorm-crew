...
#### PoC 구현 계획
- [x] Mock 데이터로 발언자 선택 로직 구현
  - selectSpeaker 함수 완성 (사회자 우선, 이전 발언자 제외, 전원 20 이하 처리)
- [x] 실제 LLM API 연동 (xAI Grok)
  - 3명 에이전트 병렬 점수 요청 성공
  - 비용 추적 기능 추가 (입력/출력 토큰별 비용 계산)
  - 평균 응답시간: 3.5초
- [ ] 메시지 히스토리 변환 로직 (에이전트별 user/assistant 분리)
  - 기본 구조는 완성, 에이전트별 관점 변환 필요
- [ ] 동적 시스템 프롬프트 생성 (손들기/토론 모드)
  - 손들기 모드: ✅ 완료
  - 토론 모드: ⏸️ 미완성
- [ ] 3개 에이전트로 토론 시뮬레이션 (발언 생성 포함)
- [ ] 검증 완료
